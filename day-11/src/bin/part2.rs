    ...
    fn do_round(&mut self) {
        for i in 0..self.monkeys.len() {
            while let Some(mut item) = self.monkeys[i].items.pop_front() {
                ...
                let modval :u64= self.monkeys.iter().map(|m| m.test_divisor).product();
...
fn operation(input: &str) -> IResult<&str, Operation> {
    ...
    let operand = if operand == "old" {
        ...
        Operand::Number(operand.parse::<u64>().expect("should parse u64"))
    };
...
fn monkey(input: &str) -> IResult<&str, Monkey> {
    ...
    let (input, items) = preceded(
        tag("  Starting items: "),
        terminated(separated_list1(tag(", "), complete::u64), newline),
    )(input)?;
    ...
    let (input, test_true_monkey) = terminated(
        preceded(tag("    If true: throw to monkey "), complete::u64).map(|v| v as usize),
        newline,
    )(input)?;
    let (input, test_false_monkey) = preceded(
        tag("    If false: throw to monkey "),
        complete::u64.map(|v| v as usize),
    )(input)?;
